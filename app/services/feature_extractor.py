...
추출하는 특징 (9가지):
    openSMILE eGeMAPSv02 (8가지):
        1. jitter   — 성대 떨림 주기의 불규칙성 (amean). 취하면 변동(방향 비일관).
        2. shimmer  — 음성 진폭의 불안정성 (amean). 취하면 변동(성별·모음 의존).
        3. hnr      — 조화음 대비 잡음 비율 (amean). 취하면 감소.
        4. f1       — 제1 포먼트 발화 내 변동성 (stddevNorm). 취하면 증가.
        5. f2       — 제2 포먼트 발화 내 변동성 (stddevNorm). 취하면 증가.
        6. loudness — 음량 변동성 (stddevNorm). 취하면 분포 확산(환경 의존).
        7. f0       — 기본 주파수 평균 (amean). 취하면 상승 경향(개인차 존재).
...
def extract_opensmile_features(wav_path: str) -> dict[str, float]:
    ...
    반환값:
        {
            ...
            "loudness": 0.35,   # 음량 변동성 (stddevNorm)
    ...
    features = {
        ...
        "loudness": float(df["loudness_sma3_stddevNorm"].iloc[0]),
